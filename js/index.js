...
//     .then((result) => {
//       console.log(result);
//       const data = renderProduct(result.data)
//       console.log(data)
//     })
//     .catch((err) => {
//       console.log(err);
...
const getDataProductAsync = async () => {
  try {
...
const renderProduct = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    const product = arr[i];
    const randomNumber = RatingRandom();
    console.log(product);
    content += `
              <div
        class="product_show w-full max-w-sm bg-neutral-primary-soft p-6 border border-default rounded-base shadow-xs"
      >
        <a href="./../html/detail.html?productid=${product.id}">
          <img
  ...
  document.querySelector(".product_line").innerHTML = content;
};
